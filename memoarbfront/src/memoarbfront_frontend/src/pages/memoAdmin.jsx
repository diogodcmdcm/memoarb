...
import GeneratePdf from "../components/reportqrcode";
...
function memoAdmin() {
  ...
  const { symbolCollection, idTemplate } = useParams(); //constante utilizada para armazenar o id da coleção ao abrir a página.
  ...
  const [creationDateTemplateNFT, setCreationDateTemplateNFT]  = useState(null);
  ...
  useEffect( async () => {    
    ...
    getNFTTemplate(symbolCollection, idTemplate);
  }, []);
  ...
  async function getNFTTemplate(sym, idT){        

    setLoading(true);      
    
    
    await memoarbfront_backend.getNftTemplate(sym, parseInt(idT)).then((result) => {
      
      setImagem(result.image);
      setNome(result.name);
      setDescricao(result.description);      
      console.log(result.supplyCap);
      console.log(result.totalSupply);
      setSupplyCap(parseInt(result.supplyCap));      
      setTotalSupply(parseInt(result.totalSupply));      
      setCreationDateTemplateNFT(Number(result.creationDateTemplateNFT));            
  ...
  async function openClaimNft(id_token){        
      let link = "https://memoprotocol.xyz/claimnftcollection/"+symbolCollection+"/"+id_token;      
      setQrCodeValue(link);
      setIsOpen(true);   
  }
      ...
      <main class="bg-[#fff9e6]" >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
              <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
                ...
                <div className="flex justify-end space-x-4">
                  <button onClick={() => generatePdfQRCode()}  className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600" >
        ...
        <br/>
          ...
          >
            {/* Div 40% */}
            <div
              className="flex-[4] h-[453px] p-5 rounded-[10px] flex-col justify-end items-start gap-2 inline-flex overflow-hidden bg-cover bg-center"
              style={{                
                backgroundImage: `url(${"https://tyk3i-wiaaa-aaaam-aec5q-cai.icp0.io"+imagem})`, // Define a imagem de fundo
                backgroundSize: "contain", // Garante que a imagem seja exibida por completo
            ...
            {/* Div 60% */}
            <div className="flex-[6] min-h-[453px] p-10 rounded-[10px] border border-[#353535] flex-col justify-start items-start gap-3.5 inline-flex overflow-hidden ">
              <a onClick={() => openClaimNft(idTemplate) } >
                <div className="px-4 py-2.5 bg-[#da80f2] rounded-lg justify-center items-center gap-4 inline-flex overflow-hidden cursor-pointer">
                  <div className="text-[#fff9e6] text-xs font-black font-['Arial'] uppercase">
                      {translations.GENERATEQRCODE}
                  </div>
                </div> 
              </a>
              <div>https://memoprotocol.xyz/claimnftcollection/{symbolCollection}/{idTemplate}</div>
              <div className="h-5 justify-start items-start gap-6 inline-flex">
                <div className="text-[#353535] text-[10px] font-bold font-['Arial'] uppercase">
                    {translations.TOTALSUPPLYMINT}:  {supplyCap}                 
                </div>
                <div className="justify-start items-start gap-1 flex">
                  <div className="w-[11px] h-[11px] px-[1.38px] py-[0.92px] origin-top-left rotate-180 justify-center items-center flex">
                    <div className="w-[8.25px] h-[9.17px] relative"></div>
                  ...
                  <div className="text-[#353535] text-[10px] font-bold font-['Arial'] uppercase">
                    {translations.MINTS}:  {totalSupply} 
                  </div>
                </div>
                <div className="justify-start items-start gap-1 flex">
                  <div className="w-[11px] h-[11px] relative"></div>
                  <div className="text-[#353535] text-[10px] font-bold font-['Arial']">
                      {translations.CREATIONDATE}: {creationDateTemplateNFT ? moment(creationDateTemplateNFT).format('DD/MM/YYYY') : '' }                                    
                  </div>
                </div>
              </div>
              ...
              <div className="w-[599px] text-[#353535] text-base font-normal font-['Arial'] whitespace-pre-wrap overflow-y-auto">
                {descricao}
              </div>
            </div>
          </div>         
